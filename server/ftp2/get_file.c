...
void get_file(int new_fd, char* username) { // 客户端上传文件
	...
	ret = recv (new_fd, &len, sizeof(len), 0); //读文件传输的长度到len字符 
	if (ret == 0) {
	...
	ret = recv_n(new_fd, (char*)&len, sizeof(len)); // 接受客户端一次文件传输的长度 4byte
	if (ret == -1) {
		printf("client close 3\n");
		goto end;
	}
	ret = recv_n(new_fd, (char*)&file_size, len); // 接受客户端传输len长度的文件
	if (ret == -1) {
	...
	while(1) {
		...
		if(ret == -1) {
			...
			ret = recv_n(new_fd, buf, len);  // 循环将len长度的内容，接受到本地buf中
			write(fd, buf, len); // 往本地磁盘里面写
			if(ret == -1) {
				printf("%5.2f%s\n", 100*sum/file_size,"%");
				break;
			}
